...
function LivePage({ credentialSessionId, preferredRoute, initialTask, session, onSession, controlSlot }: { credentialSessionId: string | null; preferredRoute: string; initialTask: string; session: Session | null; onSession: (session: Session | null) => void; controlSlot: HTMLElement | null }) {
  ...
  const [task, setTask] = useState(initialTask); const [modelId, setModelId] = useState('gemini-3.7-flash'); const [routeId, setRouteId] = useState(''); const [fallback, setFallback] = useState('gemini-3.5-flash-lite@gemini-direct'); const [safetyPolicy, setSafetyPolicy] = useState('provider_default'); const [desktopTarget, setDesktopTarget] = useState<'docker' | 'host'>('docker'); const [reasoning, setReasoning] = useState(''); const [search, setSearch] = useState(false); const [files, setFiles] = useState<Array<{ id: string; name: string }>>([]); const [handledNonce, setHandledNonce] = useState(''); const [busy, setBusy] = useState(false); const [error, setError] = useState(''); const [viewerUrl, setViewerUrl] = useState('')
  ...
  async function start(event: FormEvent) { event.preventDefault(); if (!model || !selectedRouteId || !task.trim()) return; const [fallbackModel, fallbackRoute] = fallback.split('@'); setBusy(true); setError(''); try { onSession(await api.createSession({ task: task.trim(), model: model.logicalId, primaryRoute: selectedRouteId, fallbackRoutes: fallbackModel && fallbackRoute ? [{ model: fallbackModel, route: fallbackRoute }] : [], ...(credentialSessionId ? { credentialSessionId } : {}), maxSteps: 50, ...(reasoning ? { reasoningEffort: reasoning } : {}), safetyPolicy, useBuiltinSearch: search, attachedFiles: files.map(file => file.id), retainAuditFrames: true, executionTarget: desktopTarget })) } catch (e) { setError(e instanceof Error ? e.message : String(e)) } finally { setBusy(false) } }
  ...
  const controlForm = (
    <form className="control panel" onSubmit={event => { void start(event) }}>
      ...
      <label>Task<textarea value={task} onChange={e => setTask(e.target.value)} placeholder="Describe the desktop outcome…" rows={3}/></label>
      <label>Computer Use model<select value={model?.logicalId ?? ''} onChange={e => { setModelId(e.target.value); setReasoning(''); setFiles([]) }}>{models.data?.data.map(item => <option value={item.logicalId} key={item.logicalId}>{item.displayName}</option>)}</select></label>
      <label>Primary route<select value={selectedRouteId} onChange={e => setRouteId(e.target.value)}>{model?.routes.map(route => <option value={route.id} key={route.id}>{route.provider} · {route.transport}</option>)}</select></label>
      ...
      <label>Desktop<select aria-label="Desktop target" value={desktopTarget} onChange={e => { const next = e.target.value === 'host' ? 'host' : 'docker'; setDesktopTarget(next); if (next === 'host' && safetyPolicy === 'provider_default') setSafetyPolicy('confirm_mutating') }}><option value="docker">Sandbox (Docker)</option><option value="host">Native host</option></select></label>
      {desktopTarget === 'host' && <p className="form-error" role="alert">Native host: the model clicks and types on this machine, not the sandbox. Use a test account. Confirm mutating is on by default.</p>}
      ...
      <div className="actions">{session?.status === 'RUNNING' ? <button type="button" className="danger" onClick={() => { void stop() }}><CircleStop size={17}/>Stop run</button> : <button className="primary" disabled={busy || !task.trim() || !selectedRouteId}><Play size={17}/>{busy ? 'Starting…' : 'Start run'}</button>}</div>
    </form>
  )
  return <>
    <Header eyebrow="Execution deck" title="Live agent session" aside={<div className="header-state"><Radio size={16}/><span>{stream.connected ? 'Stream linked' : 'Stream idle'}</span></div>} />
    ...
    <section className="viewport panel live-grid"><div className="panel-head"><span>{desktopTarget === 'host' ? 'Native host desktop' : 'Viewport / 1440 × 900'}</span>{session && <StatusBadge status={session.status}/>}</div><div className="screen">{desktopTarget === 'host' ? <div><MonitorPlay size={42}/><strong>Watch this machine</strong><span>The model drives your real desktop. noVNC stays on the sandbox only.</span></div> : viewerUrl ? <iframe title="Sandbox desktop" src={viewerUrl} allow="clipboard-read; clipboard-write"/> : <div><MonitorPlay size={42}/><strong>Connecting to sandbox</strong><span>The interactive desktop appears here when noVNC is ready.</span></div>}</div></section>
...
export default function App() {
  ...
  const [controlSlot, setControlSlot] = useState<HTMLDivElement | null>(null)
  ...
  function saveToken(event: FormEvent) { event.preventDefault(); setAppToken(appToken); window.location.reload() }
