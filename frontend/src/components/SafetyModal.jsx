...
export default function SafetyModal({ prompt, onDismiss }) {
  ...
  const handleRespond = useCallback(async (confirm) => {
    ...
    setResponding(true)
    try {
      await confirmSafety(prompt.sessionId, confirm, prompt.nonce)
    } catch {
  ...
  // U8: keep a ref to the latest responder so the auto-deny effect doesn't
  // capture a stale closure (and doesn't need to re-subscribe the timer).
  useEffect(() => {
  ...
  // U2: focus the safe default (Deny), trap Tab, Escape → deny, restore focus.
  useFocusTrap(!!prompt, {
    onEscape: () => handleRespondRef.current(false),
  ...
  return (
    <div ref={modalRef} className="safety-overlay" role="dialog" aria-modal="true" aria-label="Safety confirmation required" aria-describedby="safety-explanation">
      <div className="safety-modal">
        <div className="safety-header">
          <AlertTriangle size={24} className="safety-icon" />
          <h2>Safety Confirmation Required</h2>
        </div>
        <p id="safety-explanation" className="safety-explanation">{prompt.explanation || 'The agent wants to perform an action that requires your approval.'}</p>
        ...
        <div className="safety-actions">
          <button ref={denyBtnRef} className="safety-btn safety-btn-deny" onClick={() => handleRespond(false)} disabled={responding}>
            Deny
          </button>
