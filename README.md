...
A local, single-user workbench for provider-native Computer Use agents. The current release is **v3.0.2**. The operator surface is the typed `/api/v2` contract, deterministic route fallback, SQLite audit history, binary frame streaming, declarative workflows, and a five-tab React dashboard.
...
See [TECHNICAL.md](TECHNICAL.md), [v3.0.2 release notes](docs/release-notes-v3.0.2.md), [Migration](docs/migration-v2.md), [Rollback](docs/rollback-v2.md), and [Security](SECURITY.md).
